    ...
    return (
      <div className="min-h-screen bg-surface-base font-sans p-6 md:p-12 space-y-8 max-w-5xl mx-auto">
        ...
        <div className="bg-surface-lowest p-8 rounded-[24px] border border-surface-high shadow-sm text-center">
          <p className="text-text-muted italic">{data?.message || "No data available."}</p>
        </div>
      </div>
    );
  }

  const { total_queries, average_latencies, verification_rates, feedback_summary } = data;
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-surface-lowest p-6 rounded-[24px] border border-surface-high shadow-[0_4px_20px_rgba(9,20,38,0.04)]">
            ...
            <p className="text-4xl font-bold text-emerald-700">{verification_rates.final_verified_percentage}%</p>
            <p className="text-xs text-text-muted mt-1">First pass: {verification_rates.first_pass_verified_percentage}%</p>
          </div>
